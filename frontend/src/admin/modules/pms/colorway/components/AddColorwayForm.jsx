...
const AddColorwayForm = () => {
  const { refreshData } = useColorways();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        alert("Colorway created successfully!");
        await refreshData();
        navigate(`${basePath}/colorways`);
      } else {
        alert("Failed to create colorway: " + data.message);
      }
    } catch (error) {
      console.error("Error creating colorway:", error);
      alert("Failed to create colorway");
    }
  };
        ...
        <div className="border border-gray-200 rounded-lg bg-white py-4 sm:py-6 mb-4">
          <div className="flex justify-between items-start border-b border-gray-200 gap-5 mb-5">
            <div className="flex-1 pb-5 px-4 sm:px-6">
              <h2 className="text-lg font-bold">Add New Colorway</h2>
              <p className="text-xs text-gray-600">
                Create a new colorway for your products
              </p>
            </div>
            ...
            <div className="flex flex-wrap text-xs gap-3 flex-shrink-0">
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="ps-2">Create Colorway</span>
                </div>
              </button>
            </div>
          </div>
...
export default AddColorwayForm;
