...
const AddProductForm = () => {
  ...
  // Upload a single color image and return its URL
  ...
  // Upload any productImages that have a file but no url yet, return final array of { url }
  const uploadPendingImages = async (images) => {
    return Promise.all(
      images.map(async (img) => {
        // Already has an uploaded url — pass through
        if (img?.url && !img.url.startsWith("data:")) {
          return { url: img.url };
        }
        // Has a file that hasn't been uploaded yet — upload now
        if (img?.file) {
          try {
  ...
  // Upload sizeChart if it has a file but no url yet, return final url string or null
  ...
  const handleSaveDraft = async () => {
    ...
    // Upload any pending images before saving draft so URLs are stored, not base64
  ...
  const handlePublish = async () => {
    ...
    const productData = {
      ...
      description: productDescription,
      body: bodyJson,
      productImages: validImages, // ← only uploaded URLs
